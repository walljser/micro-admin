<template>
  <BasicDrawer v-bind="$attrs" :loadingText="vform.loadingText" showFooter title="部门信息" width="70%"
    ...
    <vxe-form title-colon ref="xForm" title-align="right" title-width="100" :data="vform.data" :rules="vform.rules">
      <vxe-form-item title="部门名称" field="name" :item-render="{}" span="12" title-width="150">
        <template #default="{ data }">
          <vxe-input v-model="data.name" clearable></vxe-input>
        </template>
      </vxe-form-item>
      <vxe-form-item title="上级部门" field="parent" :item-render="{}" span="12" title-width="150">
        <template #default="{ data }">
          <vxe-input v-model="parentName" suffix-icon="vxe-icon--search" readonly @click="parentSelect"></vxe-input>
        </template>
      </vxe-form-item>
      <vxe-form-item title="部门类型" field="type" :item-render="{}" span="12" title-width="150">
        <template #default="{data}">
          <vxe-radio-group v-model="data.type">
            <vxe-radio-button :label="1" content="公司"></vxe-radio-button>
            <vxe-radio-button :label="2" content="部门"></vxe-radio-button>
          </vxe-radio-group>
        </template>
      </vxe-form-item>
      ...
      <vxe-form-item title="部门备注" field="notes" :item-render="{}" span="24" title-width="150">
        <template #default="{ data }">
...
const [registerDrawer, {setDrawerProps, changeLoading, closeDrawer}] = useDrawerInner(async (data) => {
  ...
  if (data.record.id) {
    ...
    if (vform.data.parent && vform.data.parent.id === '') {
      vform.data.parent = null;
    }
    vform.data.type = 2;
...
const parentName = computed(() => {
  return vform.data.parent ? vform.data.parent.name : '';
});

function parentSelect() {
  openModal(true, {
    opener: 'parent', orgType: 2, selectMode: 1,
    orgs: vform.data.parent && vform.data.parent.id ? [toRaw(vform.data.parent)] : [],
  });
}

function closeOrgModal(data) {
  if (data.opener == 'parent') {
    vform.data.parent = data.orgs[0];
  }
  console.log(data);
}
</script>
