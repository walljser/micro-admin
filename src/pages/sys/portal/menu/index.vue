<template>
  <PageWrapper class="ztable" contentFullHeight>
    ...
    <vxe-table ref="xTree" max-height="600" :loading="tdata.loading" :data="tdata.tableData" :tree-config="{children: 'children'}">
      ...
      <vxe-column field="type" title="类型" width="60" header-align="center">
        <template #default="{ row }">
          <span v-show="row.type==='D'" class="ant-tag ant-tag-success">目录</span>
          <span v-show="row.type==='M'" class="ant-tag ant-tag-warning">菜单</span>
          <span v-show="row.type==='B'" class="ant-tag ant-tag-processing">按钮</span>
        </template>
      </vxe-column>
      ...
      <vxe-column title="操作" width="100" show-overflow header-align="center" align="right">
        <template #default="{ row }">
          <PlusSquareOutlined v-show="row.type==='D'||row.type==='M'" @click="addEvent({pid:row.id,pname:row.name},openDrawer)" style="font-size: 15px;margin-left: 10px;color: #52C41A"/>
...
async function fetch() {
  tdata.originData = await defHttp.get({url: '/sys/portal/menu/tree?porid=sys'});
  handleSearch()
}
...
async function removeEvent(id){
  ...
  if (type === 'confirm') {
    ...
    await defHttp.delete({url: '/sys/portal/menu/' + id});
    await fetch();
  }
}
