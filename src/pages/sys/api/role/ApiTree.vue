<template>
  <div class="bg-white overflow-hidden ztree">
    <div style="margin-bottom: 4px">
      ...
      <vxe-button @click="getApis">测试</vxe-button>
    ...
    <div style="border: 1px solid #ddd">
      <vxe-table
          ...
          highlight-hover-row
          :show-header="false"
          :checkbox-config="{labelField: 'name',checkRowKeys: apiArr,checkStrictly: true}"
...
// watch(props.apiArr, (ovalue, nvalue) => {
//   console.log(ovalue);
//   console.log(nvalue);
...
async function fetch() {
  tdata.originData = await defHttp.get({url: '/sys/api/main/tree'});
  handleSearch();
  // const { apis } = props;
  // if (apis && apis.length > 0) {
  //   for (const api of apis) {
  //     apiArr.value.push(api.id);
  //   }
  // }
}
...
function getApis() {
  ...
  //   apis.push({id: selectRecord2.id, name: selectRecord2.name});
  // }

  for (const selectRecord of selectRecords) {
    apis.push({id: selectRecord.id, name: selectRecord.name});
  }
  return apis;
}
