<template>
  <BasicDrawer v-bind="$attrs" :loadingText="vform.loadingText" showFooter title="角色信息" width="70%"
               @ok="handleSubmit($refs.xForm,$refs.xApiTree)" @register="registerDrawer">
    <vxe-form custom-layout title-colon ref="xForm" title-align="right" title-width="100" :data="vform.data" :rules="vform.rules">
      <Tabs type="card">
        ...
        <TabPane key="2" class="vxe-row">
          ...
          <ApiTree ref="xApiTree" :apiArr="vform.apiArr"/>
        </TabPane>
      </Tabs>
    </vxe-form>
  </BasicDrawer>
...
const [registerDrawer, {setDrawerProps, changeLoading, closeDrawer}] = useDrawerInner(async (data) => {
  ...
  vform.apiArr.splice(0, vform.apiArr.length);
  if (data.record.apis) {
    for (const api of data.record.apis) {
      vform.apiArr.push(api.id);
    }
  }
  vform.data = data.record;
  if (xApiTree && xApiTree.value && xApiTree.value.reloadTree) {
    xApiTree.value.reloadTree();
  }
  changeLoading(false);
});
...
function handleSubmit($form, $apiTree) {
  $form.validate(async function (data) {
    if (data == undefined) {
      try {
        ...
        if ($apiTree != null) {
          vform.data.apis = $apiTree.getApis();
        }
        changeLoading(true);
