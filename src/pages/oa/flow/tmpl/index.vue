<template>
  <Wrap>
    <div class='p-left'>
      <GenTree @select="handleSelect" url="/oa/flow/cate/tree"/>
    </div>
    <div class='p-right'>
...
const gridOptions = reactive<VxeGridProps>({
  columns: [
    ...
    {field: 'name', title: '模板名称', width: 300, fixed: 'left', slots: {default: 'name_default'}},
...
function handleSelect(node) {
  if(node!=null){
    form.value.catid = node.id;
    form.value.catna = node.name;
  }else{
    form.value.catid = '';
    form.value.catna = '';
  }
...
const pageAdd=()=>{
  // //vben的暂未开发好
  window.open("http://zsvg.gitee.io/vue#/page/ofte?catid=&catna=");
  // window.open(import.meta.env.VITE_PUBLIC_PATH+"#/page/ofte")
  // window.open("#/page/ofte");
}

const pageEdit=(id)=>{
  window.open("http://zsvg.gitee.io/vue#/page/ofte?id="+id)
  // window.open(import.meta.env.VITE_PUBLIC_PATH+"#/page/ofte?id="+id)
  // window.open("#/page/ofte");
}
